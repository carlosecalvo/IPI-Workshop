...
The MonteCarlo package[^1] for the R language provides tools to create simulation studies quickly and easily and it also allows to summarize the results in LaTeX tables. This vignette gives details on the implementation of the MonteCarlo package and presents examples for its application.

[^1]: See: MonteCarlo package <https://cran.r-project.org/web/packages/MonteCarlo/>
...
Then, we define the combinations of parameters that we are interested in and collect them in a list. The elements of this list must have the same names as the parameters for which we want to supply grids.
...
From the summary, we can see that the simulation results are stored in an array of dimension `c(4,6,2,1000)`. The Monte Carlo repetitions are collected in the last dimension of the array. Up to this point there is no "reasonable" way to see the results.
...
If *func* returns a list with more than one element, then each of the list elements has its own column in the dataframe
...
```{r plot_ttest}
tbl <- as_tibble(df)
ggplot(filter(tbl, loc==0, scale==1)) + 
  geom_density(aes(x=stat, col=factor(n)))
```

```{r}
ggplot(tbl) +
  geom_freqpoly(aes(x = stat, col = factor(loc)))
```

```{r}
ggplot(tbl) +
  geom_point(aes(x = stat, y = decision, col = factor(n))) +
  facet_wrap(facets = ~n)
```
...
Here, we consider a second example: the evaluation of estimators. In particular, we compare the *mean()* and the *median()* as estimators for the expected value of a Gaussian random variable. This example serves to illustrate the use of the optional arguments of the *MakeTable()* function.

First, we specify the function of interest: *mean_vs_median()* (defined below) generates a sample of *n* normal distributed random variables with standard deviation *scale*. It then calculates the *mean()* and the *median()* and returns both in a list.
...
We can check the summary of the simulation:
...
Again, the default behavior of *MakeTable()* is to print separate tables for each value in the list returned by *func*. 
...
### Non-nested data Generating Process
...
However, this does not mean that *MonteCarlo()* cannot handle non-nested data generating processes. As an example, we consider the case where we want to compare the standard deviation of the *mean()* and the *median()* as we did above, but for samples from a normal distribution and from a uniform distribution.

These are clearly non-nested. To deal with this situation, we include *DGP* as an additional argument in *mean_vs_median()* and create a case distinction, so that the value of *DGP* determines from which distribution the sample is generated. We then include the possible values of *DGP* in an additional parameter grid.

```{r mean_median_unnested}
...
Note that the width of the uniform distribution is specified so that both the normal and the uniform distribution have a standard deviation of *scale*.


```{r erg_m_m, results='hide'}
erg_mean_median <- MonteCarlo(func = mean_vs_median, nrep=1000, param_list=param_list)
```
...
We can the explore the data with *MakeFrame()* and plot it according to the needs of our analysis.

```{r}
df <- as_tibble(MakeFrame(erg_mean_median))
head(df)
```

```{r}
ggplot(df) +
  geom_freqpoly(aes(x = median, col = factor(scale))) +
  facet_wrap(~DGP)
```
```{r}
ggplot(df) +
  geom_freqpoly(aes(x = mean, col = factor(scale))) +
  facet_wrap(~DGP)
```


These examples shows that *MonteCarlo()* is very versatile. Depending on the specification of *func*, it is possible to deal with nested or non-nested DGPs. Similarly, multivariate outputs could be handled by splitting them in several elements and returning them seperately. 


## Appendix: The MonteCarlo() Functions
